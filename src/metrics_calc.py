...
def compute_metrics(class_ontology_dict:dict, prop_ontology_dict:dict) -> dict:
    ...
    # Calculate ADIT-LN
    adit_ln, nol = calc_adit_ln_metrics(class_ontology_dict, prop_ontology_dict, inheritance_g)
    ...
    avg_num_shortest_paths, avg_shortest_path, diameter_inheritance_g = calc_shortest_path_metrics(inheritance_g)
    avg_num_shortest_paths_full, avg_shortest_path_full, diameter_ont_g = calc_shortest_path_metrics(ont_graph)

    return {"Number of classes": calc_num_nodes(inheritance_g),
                ...
                "Average shortest path length, full graph": avg_shortest_path_full,
                "Diameter of full graph": diameter_ont_g,
                "Number of inheritance relationships": calc_num_inheritance_edges(inheritance_g),
...
def extract_inheritance_graph(class_ontology_dict:dict, prop_ontology_dict:dict) -> nx.MultiDiGraph:
...
def extract_property_graph(class_ontology_dict:dict, prop_ontology_dict:dict) -> nx.MultiDiGraph:
...
def calc_adit_ln_metrics(class_ontology_dict:dict, prop_ontology_dict:dict, inheritance_g:nx.MultiDiGraph) -> tuple[float, int]:
    ...
    for class_key, class_rep in class_ontology_dict.items(): 
        if (len(class_rep.superclass_list) == 1) and class_rep.superclass_list[0] == 'owl:Thing':
            root_class_list.append(class_key)
        else:
            for superclass_key in class_rep.superclass_list:
                superclass_name = superclass_key
                
                if (superclass_name not in literals_dict) and (superclass_name in leaf_class_list):
    ...
    for i, root_class in enumerate(root_class_list):
        for j, leaf_class in enumerate(leaf_class_list):
            if nx.has_path(inheritance_g, source=root_class, target=leaf_class):
                paths_gen = nx.all_simple_paths(inheritance_g, source=root_class, target=leaf_class)
...
def calc_shortest_path_metrics(ont_graph:nx.MultiDiGraph) -> tuple[float, float, int]:
    ...
    shortest_path_lengths = [[len(list(path)) for path in s_paths[key].values()] for key in s_paths.keys()]

    diameter = np.max(np.max([path_list for path_list in shortest_path_lengths]))

    return num_s_paths/calc_num_nodes(ont_graph), np.sum(np.sum(shortest_path_lengths))/num_s_paths, diameter
...
def calc_relationship_richness(prop_graph:nx.MultiDiGraph, ont_graph:nx.MultiDiGraph) -> float:
