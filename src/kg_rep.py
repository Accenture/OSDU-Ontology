...
class PropertyRep:
    ...
    def verify_match(self, range_name: str, prop_type: str, replace_range=True) -> None:
        ...
        if self.type != prop_type:
            raise Exception(
                "Incorrect property type " + prop_type, ". Type should be " + self.type
            )
...
class ClassRep:
    ...
    def add_superclass(self, superclass: str, process_name_flag=True) -> None:
        ...
        if process_name_flag:
            processed_superclass = process_name(
                extract_classname_from_filename(superclass)
            )
        else:
        ...
        ) and ("owl:Thing" in self.superclass_list):
            self.superclass_list.remove("owl:Thing")
            self.superclass_list.append(processed_superclass)
        elif (
            (superclass == "owl:Thing")
            and ("ReferenceData" not in self.superclass_list)
            and ("MasterData" not in self.superclass_list)
            and ("Dataset" not in self.superclass_list)
            and ("WorkProductComponent" not in self.superclass_list)
            and ("Abstract" not in self.superclass_list)
            and ("owl:Thing" not in self.superclass_list)
        ):
            self.superclass_list.append(superclass)
        elif (
            (superclass != "")
            and (processed_superclass not in self.superclass_list)
            and (self.name != processed_superclass)
        ) and (superclass != "owl:Thing"):
    ...
    def add_subclass(self, subclass_name: str, process_name_flag=True) -> None:
        if process_name_flag:
            processed_subclass = process_name(
                extract_classname_from_filename(subclass_name)
            )
        else:
    ...
    def add_array_property_restriction(
        ...
        array_prop = {
            "prop_name": process_prop_name(property_name),
            "min_card": min_card,
...
def add_property_from_parameters(
    property_name: str,
    domain_name: str,
    range_name: str,
    ontology_dict: dict,
    ...
    if process_prop_name(property_name) in ontology_dict:
        ...
        try:
            ontology_dict[property_name].verify_match(
                range_name, property_type, replace_range
            )
            ...
            if verbose:
                print(e)
                print(
                    "\nTried to create property on top of a class:",
                    property_name,
                    domain_name,
                )
    else:
