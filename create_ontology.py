...
def __main__():
    ...
    parser = argparse.ArgumentParser(description="Specify schema directory and mode",
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-s", "--src", help="Source location for schema files", default="osdu-ontology-generator/osdu_full_schema/")
    parser.add_argument("-d", "--dest", required=False, default=curr_path+"/", help="Destination location for output ttl")
    parser.add_argument("-o", "--ontology-name", required=False, default="osdu", help="Key descriptor for ontology name")
    parser.add_argument("-v", "--verbose", required=False, action="store_true", default=True, help="increase verbosity")
    parser.add_argument("-m", "--report_metrics", required=False, default=False, action="store_true")
    ...
    assemble_ttl(CLASS_ONTOLOGY_DICT, PROP_ONTOLOGY_DICT, 
    URL_TO_CLASSNAME_DICT, 
    ARRAY_PROPERTIES_DICT,
    dest_filepath=args.dest, write_file=(not args.report_metrics))
...
def add_class_from_schema_dict(key:str, schema:dict, verbose:bool=False):
    ...
    if ("properties" in schema) and \
        ("id" in schema["properties"]) and \
        ("kind" in schema["properties"]) and \
        ("legal" in schema["properties"]) and \
        ("meta" in schema["properties"]) and \
        ("version" in schema["properties"]) and \
        ("tags" in schema["properties"]) and \
        (class_name != "AbstractSystemProperties"):
    ...
    if ("properties" in schema) and ("data" in schema["properties"]):
        ...
        for property_dict in properties_list:
            ...
            if "$ref" in property_dict:
                ...
                add_class_from_schema_dict(process_name(property_dict["title"]), 
                            property_dict)
    ...
    if "required" in schema:
        for prop in schema["required"]:
            if (lower_process_name(prop) not in ["kind", "legal", "acl"]) \
                    or ("AbstractSystemProperties" not in superclasses):
                add_array_property_restriction(class_name, lower_process_name(prop), min_card=1)
    ...
    CLASS_ONTOLOGY_DICT = add_class_from_parameters(class_name, superclasses, 
                            CLASS_ONTOLOGY_DICT, comments, pref_label=title, 
                            subclass_list=subclasses)

    return 

def add_property_from_schema_dict(property_name:str, property_dict:dict, 
        class_name:str, verbose:bool=False) -> (dict, dict, dict):
    ...
    if ("oneOf" in property_dict):
            ...
            if "type" in option_dict:
                ...
                if len(list(option_dict.keys())) == 1:
                    if option_dict["type"] != "null":
                        ...
                        prop_type = PropType.Object if option_dict["type"] == "object" else PropType.Datatype
                elif "title" in option_dict:
                    add_class_from_schema_dict(process_name(option_dict["title"]), option_dict)
                ...
                PROP_ONTOLOGY_DICT = add_property_from_parameters(
                    ...
                    property_type=prop_type, replace_range=False)
            else:
                print("No type key in oneOf instance:",option_dict)

    elif ("x-osdu-relationship" in property_dict) and \
        (property_name[-2:] == "ID") and (property_dict['x-osdu-relationship'] != []) \
        ...
        try:
            PROP_ONTOLOGY_DICT = add_property_from_parameters(
                    property_name='has' + upper_split_camelcase(property_dict['x-osdu-relationship'][0]["EntityType"]), 
                    domain_name=class_name,
        ...
        CLASS_ONTOLOGY_DICT = add_class_from_parameters(
                ...
                superclass_list=['owl:Thing'], ontology_dict=CLASS_ONTOLOGY_DICT, comments=[]
            )
        ...
        PROP_ONTOLOGY_DICT = add_property_from_parameters(
                ...
                range_name='string', 
                ontology_dict=PROP_ONTOLOGY_DICT,
                comment=property_dict["description"] if "description" in property_dict else '',
        ...
        PROP_ONTOLOGY_DICT = add_property_from_parameters(
                property_name='has' + upper_split_camelcase(re.sub("\ ", "", property_name)), 
                domain_name=class_name,
        ...
        if ('type' in property_dict) and ((property_dict["type"] == "array") or ("items" in property_dict)):
            if ("minItems" in property_dict):
                if property_dict["items"]["type"] == "object":
                    add_array_property_restriction(class_name,
                        property_name, 
                    ...
                    new_class_name = class_name+upper_split_camelcase(property_name)+"Array"
                    CLASS_ONTOLOGY_DICT = add_class_from_parameters(
                    ...
                    PROP_ONTOLOGY_DICT = add_property_from_parameters(
                        property_name=property_name, domain_name=class_name, range_name=new_class_name, 
                        ontology_dict=PROP_ONTOLOGY_DICT, replace_range=False
                    )
                    ...
                    add_property_from_schema_dict(property_name="items", property_dict=property_dict["items"], class_name=new_class_name)
            ...
            if "$ref" in property_dict["items"]:
                ...
                new_class_name = property_dict["items"]["title"] if "title" in property_dict["items"] else property_name
                new_class_name = upper_split_camelcase(strip_whitespace(new_class_name))

                PROP_ONTOLOGY_DICT = add_property_from_parameters(
                        property_name='has' + upper_split_camelcase(strip_whitespace(property_name)), 
                        domain_name=class_name,
                ...
                for superclass_dict in property_dict["items"]["allOf"]:
                    if "$ref" in superclass_dict:
                        ...
                        add_class_from_schema_dict(process_name(superclass_dict["title"]), 
                            superclass_dict)
                        superclasses.append(process_name(superclass_dict["title"]))
                CLASS_ONTOLOGY_DICT= add_class_from_parameters(new_class_name, 
                    superclasses, CLASS_ONTOLOGY_DICT)
                PROP_ONTOLOGY_DICT = add_property_from_parameters(
                                        property_name='has'+upper_split_camelcase(property_name), 
                                        domain_name=class_name, 
                                        range_name=new_class_name, 
                ...
                new_class_name = property_dict["items"]["title"] if "title" in property_dict["items"] else property_name
                new_class_name = process_name(new_class_name)
                CLASS_ONTOLOGY_DICT = add_class_from_parameters(new_class_name, ["owl:Thing"], CLASS_ONTOLOGY_DICT)
                PROP_ONTOLOGY_DICT = add_property_from_parameters(property_name, class_name, new_class_name, PROP_ONTOLOGY_DICT)
                for new_class_dict in property_dict["items"]["oneOf"]:
                    if "$ref" in new_class_dict:
                        ...
                        if process_name(new_class_dict["$ref"]) in CLASS_ONTOLOGY_DICT:
                            CLASS_ONTOLOGY_DICT[process_name(new_class_dict["$ref"])].add_superclass(new_class_name)
                        else:
                            CLASS_ONTOLOGY_DICT = add_class_from_parameters(process_name(new_class_dict["$ref"]), \
                                [new_class_name], CLASS_ONTOLOGY_DICT)
                    #    superclasses.append( superclass_dict["$ref"])
                    else:
                        add_class_from_schema_dict(process_name(new_class_dict["title"]), 
                            new_class_dict)
                        CLASS_ONTOLOGY_DICT[process_name(new_class_dict["title"])].add_superclass(new_class_name)
                ...
                #Call property creation again with prefix+Array class as the domain
                add_property_from_schema_dict(property_name="items", 
                    property_dict=property_dict["items"], class_name=new_class_name)

                return 

            else:
                range_name = property_dict["items"]["type"]
                pattern = property_dict["items"]["pattern"] if "pattern" in property_dict["items"] else ""
...
def add_array_property_restriction(class_name:str, property_name:str, on_class:str="", min_card:int=1):
